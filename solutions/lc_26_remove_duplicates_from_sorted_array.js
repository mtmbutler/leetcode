...
var removeDuplicates = function(nums) {
    ...
    let last = nums[0];
    while (true) {
        if (i >= nums.length) {break;}  // Break at the end of the array
        if (nums[i] === last) {nums.splice(i, 1);}  // Pop if equal to previous
        else {last = nums[i]; i++;}  // Keep iterating
    }
    return i;
};
...
if (typeof module != 'undefined' && !module.parent) {
    ...
    for (let case_ of test_cases) {
        let arg = case_[0];
        let out = case_[1];
        let altered_arg = case_[2];
        console.log(arg, out);
        let result = removeDuplicates(arg);
        console.log(arg, result);
        console.assert(_.isEqual(result, out));
        console.assert(_.isEqual(arg, altered_arg));
    }
}
